...
Quy trình được chia làm hai phần rõ rệt: **(A) Dành cho người chia sẻ** (người có thư mục hoặc máy in cần chia sẻ) và **(B) Dành cho người kết nối** (người cần truy cập vào tài nguyên đó).

### A. Dành cho Người Chia sẻ (Trên máy chủ)

#### Bước 1: Chuẩn bị

1.  Tải về toàn bộ mã nguồn từ GitHub.
2.  Để chia sẻ thư mục, hãy đảm bảo bạn có 2 tệp `Chay-Tao-Chia-Se.bat` và `Tao-Chia-Se.ps1` nằm **cạnh nhau** trong cùng một thư mục.
3.  Tương tự, để chia sẻ máy in, hãy có 2 tệp `Chay-ChiaSe-MayIn.bat` và `ChiaSe-MayIn.ps1`.

#### Bước 2: Chạy chương trình với quyền Admin

-   Nhấp chuột phải vào tệp `.bat` bạn muốn chạy (ví dụ `Chay-Tao-Chia-Se.bat`).
-   Chọn **"Run as administrator"**.
-   Một cửa sổ UAC (User Account Control) sẽ hiện ra, hãy chọn **"Yes"** để cấp quyền.

#### Bước 3: Làm theo hướng dẫn trên màn hình

Một cửa sổ PowerShell sẽ mở ra và hướng dẫn bạn từng bước:

1.  **Chọn tài nguyên:**
    -   **Với ổ đĩa:** Một cửa sổ đồ họa sẽ hiện ra. Hãy duyệt và chọn thư mục bạn muốn chia sẻ rồi nhấn **OK**.
    -   **Với máy in:** Chương trình sẽ liệt kê danh sách các máy in đang có. Bạn chỉ cần **nhập số thứ tự** tương ứng và nhấn `Enter`.

2.  **Đặt tên chia sẻ:**
    -   Chương trình sẽ gợi ý một tên chia sẻ (thường là tên của thư mục/máy in).
    -   Bạn có thể nhấn `Enter` để đồng ý, hoặc gõ một tên mới rồi nhấn `Enter`.

3.  **Xác nhận:**
    -   Chương trình sẽ hiển thị lại thông tin để bạn xác nhận lần cuối.
    -   Nhấn `Enter` để bắt đầu quá trình chia sẻ tự động.

#### Bước 4: Lấy và gửi Gói Cài đặt

-   Sau khi chạy xong, chương trình sẽ thông báo thành công và **tự động mở thư mục chứa Gói Cài đặt** cho bạn.
-   Thư mục này có tên dạng `Goi_Ket_Noi_[Tên Chia sẻ]` và nằm trên **Desktop** của bạn.

-   **Nén thư mục này lại** (Nhấp chuột phải -> Send to -> Compressed (zipped) folder).
-   Gửi tệp `.zip` này cho đồng nghiệp, bạn bè của bạn qua Zalo, Email, USB...

---

### B. Dành cho Người Kết nối (Trên máy khách)

#### Bước 1: Nhận và giải nén

-   Bạn sẽ nhận được một tệp `.zip` từ người chia sẻ.
-   Hãy giải nén nó ra một thư mục (Nhấp chuột phải -> Extract All...).

#### Bước 2: Chạy tệp `.bat`

-   Mở thư mục vừa giải nén. Bên trong sẽ có 2 tệp: `_Logic.ps1` và một tệp `.bat` (ví dụ `KetNoiOdiaMang.bat`).
-   **Chỉ cần nhấp đúp chuột vào tệp `.bat`**.

#### Bước 3: Cấu hình lần đầu (Chỉ một lần duy nhất)

-   **Lần đầu tiên** bạn chạy script trên máy, nó sẽ cần cấu hình hệ thống (tắt IPv6, sửa các chính sách bảo mật...).
-   Nó sẽ yêu cầu quyền Administrator. Hãy chọn **"Yes"** khi cửa sổ UAC hiện ra.
-   Sau khi cấu hình xong, nó sẽ yêu cầu bạn **KHỞI ĐỘNG LẠI MÁY TÍNH**. Đây là bước bắt buộc.

#### Bước 4: Chạy lại script để kết nối

-   Sau khi đã khởi động lại máy, hãy quay lại thư mục đã giải nén.
-   **Chạy lại tệp `.bat` một lần nữa**.
-   Lần này, script sẽ không yêu cầu quyền Admin nữa. Nó sẽ tiến hành kiểm tra kết nối và tự động thêm ổ đĩa mạng/máy in vào máy của bạn.
-   Khi thấy thông báo **"THANH CONG"**, bạn đã hoàn tất!
    -   Ổ đĩa mạng sẽ xuất hiện trong **This PC**.
    -   Máy in sẽ xuất hiện trong **Printers & Scanners** (trong Settings).
